                     ...
                     <div class="container">
                        <div class="row">
                           <div class="col-md-12 blogInfo mb-3 p-0">
                              ...
                              <span class="ml-2 heartBtn" ><i class="fa-solid fa-heart mr-1"></i><strong class="likes">0</strong> Likes</span>
      ...
      $(document).on('click','.view-blog', function(){
            ...
            $.ajax({
               ...
               success: function (data) {
                     ...
                     $('#blogTitle').text(data.blog.blog_title);
                     $('#blogImage').attr('src',data.blog.blog_image);
                     $('#blogDescription').text(data.blog.blog_description);
                     $('#blogDescription').text(data.blog.blog_description);
                     $('#publishDate').text(data.blog.publish_date);
                     $('#blog_id').val(data.blog.id);

                     $.each(data.blog.comments, function (i) {
                        var replyForm = '<div class="form-section'+data.blog.comments[i].id+'" style="display:none">'+
                                             '<form class="reply-form">'+
                                                '<div class="form-group">'+
                                                   '<textarea name="reply" placeholder="Enter reply here..." id="reply'+data.blog.comments[i].id+'" class="reply-cmnt"></textarea>'+
                        ...
                        if( data.blog.comments[i].replies && data.blog.comments[i].replies.length > 0){ 
                           cmtDiv ="<div class='comment-replies bg-light p-3 mt-3 rounded'>"+
                                          "<h6 class='comment-replies-title mb-4 text-muted text-uppercase'> "+data.blog.comments[i].replies.length + " replies</h6>" +
                                          "<div class='reply"+data.blog.comments[i].id+" mb-4'>" + 
                        ...
                        var comment ="<div class='comment'> " +
                                       "<div class='flex-grow-1 ms-2 ms-sm-3'> " +
                                          "<div class='comment-meta mt-3 d-flex align-items-baseline'>"+
                                             "<h5 class='me-2 font-weight-bold'> "+ data.blog.comments[i].user.name +" </h5> <span class='ml-1'> -  "+data.blog.comments[i].created_at+"</span>" + 
                                          "</div>" +
                                          "<div class='comment-body'> "+ data.blog.comments[i].comment +" </div>"+                     
                                          "<div class='reply-comment'>" +
                                             "<button class='btn btn-warning reply-btn text-white' id='replyBtn"+data.blog.comments[i].id+"' style='font-size:14px' data-parentId='"+data.blog.comments[i].id+"'>Reply <i class='fa fa-reply'></i></button>" +
                                             replyForm +
                                          "</div>" +
                                         cmtDiv
                                          +
                        ...
                        if(data.blog.comments[i].replies !== null && data.blog.comments[i].replies.length > 0){
                           // var replyDiv = 'test ';
                           $.each(data.blog.comments[i].replies, function (key, val) {
                     ...
                     var name = '';
                     jQuery.isEmptyObject(data.blog.user) ? name = 'Admin' : name = data.blog.user.name ;
                     $('#blogBy').text(name);

                     if(data.avg !== null){
                        let avg = Math.round(data.avg);
                        for(var i=1; i <= avg; i++){
                           $('.start-'+ i ).css("color","#fc0");
                        }
                     }
                     ...
                     console.log(auth_id);
                     if(data.blog.likes !== null){
                        $.each(data.blog.likes, function (key,val) {
                           if(val.blog_id === data.blog.id && val.user_id == parseInt(auth_id) ){
        ...
        let blog_id = $('#blog_id').val();
         $.ajax({
            ...
            success: function(response) { 
               console.log(response); 
               if(response.status == 200){
                  var count = $('.likes').text();
                  if(response.message !== 'Dislike !'){
                     $('.fa-solid').css('color','red');
                     $('.fa-solid').css('opacity','100%');
                     $('.likes').text(parseInt(count)+1);    
                  }else{
                     $('.fa-heart').css('color','#76838f');
                     $('.likes').text(parseInt(count)-1);    
                  }
